...
def convert_dataset_to_coco_format(dataset_paths, output_dir):
    ...
    for split, dataset_path in dataset_paths.items():
        coco_format = get_coco_from_labelme_folder(dataset_path, coco_category_list=train_coco.json_categories)
        save_json(data=coco_format.json, save_path=os.path.join(output_dir, f"{split}.json"))
...
def build_mm_dataloader(config, datasets, split="train"):
    ...
    if split == "train":
        ...
        train_dataloaders = [build_dataloader(dataset, **train_dataloader_config) for dataset in datasets]
        return train_dataloaders
    elif split == "test":
        test_dataloader_config = config.data.get("test_dataloader", {})
        if not test_dataloader_config:
            test_dataloader_config = dict(samples_per_gpu=1, workers_per_gpu=2, dist=False, shuffle=False)
